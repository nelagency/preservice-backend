...
export class EmailService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    try {
      ...
      this.log.log(
        `[Email] SMTP prêt en mode ${process.env.GMAIL_USER ? 'Gmail' : 'SMTP générique'} (from=${this.from})`,
      );
    } catch (e: any) {
...
<body style="margin:0;background:#f6f7fb;padding:24px;font-family:Arial,Helvetica,sans-serif;">
  ...
  async participationApproved(
    ...
    const html = this.layoutHTML({
      title: subject,
      greeting:
        `Bonjour ${[serveur.nom, serveur.prenom].filter(Boolean).join(' ') || ''}`.trim() ||
        'Bonjour,',
      intro:
        opts?.intro ??
  ...
  async participationRequestedAdmin(
    to: string,
    event?: EventLike,
    serveur?: ServeurLike,
  ) {
    await this.generic(to, `Demande de participation reçue`, {
      ...
      details: {
        Serveur:
          [serveur?.nom, serveur?.prenom].filter(Boolean).join(' ') || '—',
        Évènement: event?.title ?? '—',
        Date: this.dateOnly((event as any)?.startdate ?? (event as any)?.date),
      },
  ...
  async timesheetReviewedServeur(
    to: string,
    ...
    await this.generic(
      to,
      status === 'approved' ? 'Feuille approuvée' : 'Feuille rejetée',
      {
        intro:
          status === 'approved'
  ...
  async timesheetPaidServeur(to: string, amount: number, finalize: boolean) {
    await this.generic(
      to,
      finalize ? 'Paiement finalisé' : 'Paiement enregistré',
      {
        intro: finalize
