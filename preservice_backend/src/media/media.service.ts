...
export class MediaService {
  constructor(
    ...
    @InjectModel(BeforeAfterPair.name)
    private pairModel: Model<BeforeAfterPairDocument>,
  ...
  async finalizeR2Upload(body: {
    ...
    const doc = await this.mediaModel.create({
      ...
      uploader: body.uploaderId
        ? new Types.ObjectId(body.uploaderId)
        : undefined,
  ...
  async deleteAsset(assetId: string) {
    ...
    if (pairIds.length) {
      ...
      await this.eventModel.updateMany(
        { beforeAfter: { $in: pairIds } },
        { $pull: { beforeAfter: { $in: pairIds } } },
      );
    }
