...
let NotificationsService = NotificationsService_1 = class NotificationsService {
    ...
    async pushToUsers(args) {
        ...
        for (const u of users) {
            if (!u.email)
                continue;
            this.sendEmailFor(args.type, u.email, args.payload).catch((e) => this.log.warn(`email fail (${args.type}) -> ${u.email}: ${e?.message || e}`));
        }
    ...
    async pushToServeurs(args) {
        ...
        for (const s of serveurs) {
            if (!s.email)
                continue;
            this.sendEmailFor(args.type, s.email, args.payload).catch((e) => this.log.warn(`email fail (${args.type}) -> ${s.email}: ${e?.message || e}`));
        }
        return { created: docs.length };
    }
    ...
    async listMine(userId, limit = 50, before) {
        const q = { user: new mongoose_2.Types.ObjectId(userId) };
        if (before)
...
exports.NotificationsService = NotificationsService = NotificationsService_1 = __decorate([
    ...
    __param(3, (0, mongoose_1.InjectModel)(Event.name)),
...
//# sourceMappingURL=notifications.service%20copy.js.map