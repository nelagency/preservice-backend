...
import {
  Timesheet,
  TimesheetDocument,
} from 'src/timesheets/entities/timesheet.entity';
...
export class NotificationsService {
  ...
  async pushToUsers(args: PushBase & { userIds: string[] }) {
    ...
    const docs = await this.notifModel.insertMany(
      users.map((u) => ({
        ...
        actor:
          args.actorId && Types.ObjectId.isValid(args.actorId)
            ? new Types.ObjectId(args.actorId)
            : undefined,
    ...
    const emailJobs = users
      ...
      .map((u) =>
        this.sendEmailFor(args.type, u.email, args.payload, 'User').catch((e) =>
          this.log.warn(
            `email fail (${args.type}) -> ${u.email}: ${e?.message || e}`,
          ),
        ),
      );
  ...
  async pushToServeurs(args: PushBase & { serveurIds: string[] }) {
    const ids = (args.serveurIds || []).filter((x) =>
    ...
    const emailJobs = serveurs
      .filter((s) => !!s.email)
      .map((s) =>
        this.sendEmailFor(args.type, s.email, args.payload, 'Serveur', s).catch(
          (e) =>
            this.log.warn(
              `email fail (${args.type}) -> ${s.email}: ${e?.message || e}`,
            ),
        ),
      );
    await Promise.allSettled(emailJobs);
  ...
  private async resolveContext(payload?: any) {
    ...
    const [event, serveur, ts] = await Promise.all([
      eventId && Types.ObjectId.isValid(eventId)
        ...
        : null,
      serveurId && Types.ObjectId.isValid(serveurId)
        ? this.serveurModel.findById(serveurId).lean()
        : null,
      timesheetId && Types.ObjectId.isValid(timesheetId)
  ...
  private async sendEmailFor(
    ...
    switch (type) {
      ...
      case 'TIMESHEET_REVIEWED':
        return this.mail.timesheetReviewedServeur(
          to,
          payload?.status,
          payload?.comment,
        );
      ...
      case 'TIMESHEET_PAID':
        return this.mail.timesheetPaidServeur(
          to,
