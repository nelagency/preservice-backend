...
export class TimesheetsService {
  constructor(
    ...
    private partModel: Model<ParticipationDocument>,
  ...
  async submitForEvent(
    ...
    if (!isTermine && eventDate > now)
      throw new ForbiddenException('Timesheet not allowed before event end');
    ...
    if (!approvedPart)
      throw new ForbiddenException('Not an approved participant');
    ...
    const workedMinutes = this.computeWorkedMinutes(
      start,
      end,
      dto.breakMinutes,
    );
    if (workedMinutes <= 0)
      throw new BadRequestException('worked minutes must be > 0');
  ...
  async review(
    ...
    if (serveurIdStr) {
      await this.notifications.pushToServeurs({
        ...
        actorModel: 'User',
        title:
          ts.status === 'approved' ? 'Feuille approuvée' : 'Feuille rejetée',
  ...
  async listForServeur(serveurId: string) {
    ...
    const sid = Types.ObjectId.isValid(serveurId)
      ? new Types.ObjectId(serveurId)
      : serveurId;
  ...
  async listForServeurSmart(userOrServeurId: string) {
    const id = Types.ObjectId.isValid(userOrServeurId)
      ? new Types.ObjectId(userOrServeurId)
      : userOrServeurId;
    // 1) essai direct (le sub est déjà un Serveur._id)
    const list = await this.tsModel
      .find({ serveur: id as any })
      .populate('event')
      .sort({ createdAt: -1 })
      .lean()
      .exec();
